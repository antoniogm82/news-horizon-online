...
CREATE INDEX IF NOT EXISTS idx_posts_published_date ON public.posts(published_at DESC) WHERE published = true;
CREATE INDEX IF NOT EXISTS idx_posts_keywords ON public.posts USING GIN(keywords);
CREATE INDEX IF NOT EXISTS idx_posts_canonical ON public.posts(canonical_url);
